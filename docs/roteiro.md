...
- valores remotos existem e são exibidos (`RcBridge.floatValue` +
  `createTextFromFloat`);
- **o player reavalia continuamente** — o relógio usa `hour()`, `minutes()` e
  `seconds()` e os segundos avançam em tempo real, verificado por dois
  screenshots com cinco segundos de intervalo. Isso é a base do item B4.
- nenhum byte trafega durante os toques (medido no log do Ktor).

**O que não funciona:** `setValue` dentro de `onClick`, e `StateUpdater`
chamado de fora. Seis hipóteses testadas, cinco descartadas com evidência.

**→ A investigação completa está em
[`diario/03-estado-remoto.md`](diario/03-estado-remoto.md).**

**Próximo teste, em ordem de promessa:** `RcFloat.flush()`,
`player.updateDocument()`, e procurar o opcode 222 no dump binário.
### B3. Expressões 🔜
